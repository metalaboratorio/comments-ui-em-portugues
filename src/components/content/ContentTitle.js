...
const Count = ({showCount, count}) => {
    ...
    if (count === 1) {
        return (
            <div className="text-[1.6rem] text-[rgba(0,0,0,0.5)] dark:text-[rgba(255,255,255,0.5)]">1 commentário</div>
        );
    }

    return (
        <div className="text-[1.6rem] text-[rgba(0,0,0,0.5)] dark:text-[rgba(255,255,255,0.5)]">{formatNumber(count)} comentários</div>
    );
};
...
const Title = ({title}) => {
    if (title === null) {
        return (
            <><span className="hidden sm:inline">Inscritos </span><span className="capitalize sm:normal-case">discussão</span></>
        );
    }
